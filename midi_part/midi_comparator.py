...
midi_created = pretty_midi.PrettyMIDI('./Sounds/detected_notes.mid')
instruments = []
...
fig, axes = plt.subplots(2, nbInst+1, figsize=(6*(nbInst+1), 12))

if nbInst == 1:
    axes = axes.reshape(2, nbInst+1)
...
for j, (name, _, created_count, pExact, p1, p12, _, _) in enumerate(instruments):
    ...
    for bar, value in zip(bars, [pExact, p1, p12]):
        txt = f"{value}\n({((value* 100) / len(inst_created.notes)):.1f}%)"
        ax.annotate(txt,
                    xy=(bar.get_x() + bar.get_width()/2, bar.get_height()),
                    xytext=(0,3), textcoords="offset points",
                    ha="center", va="bottom")
